...
months=DateRange[{1979,1,1},{1995,12,1},"Month"];
...
{dlat,dlon}=Block[{tempt=Import[DateString[months[[1]],{"CONUS_","Year","_","Month","_",var[[1]],".mx"}]]},{tempt["lat"],tempt["lon"]}];
...
DynamicNet=Block[{c=32},
	NetGraph[
 <|"resize0"->{Table[{ConvolutionLayer[c,{2,3,3}],BatchNormalizationLayer[],ParametricRampLayer[]},2],FlattenLayer[1],ResizeLayer[{40,80}]},
   "cate0"->CatenateLayer[],
   "up0"->{Table[{ConvolutionLayer[2c,{3,3},"PaddingSize"->1],BatchNormalizationLayer[],ParametricRampLayer[]},3]},
   "output0"->{ConvolutionLayer[1,{1,1}],BatchNormalizationLayer[],Ramp},
   
   "resize1"->DeconvolutionLayer[c,{2,2},2],
   "cate1"->CatenateLayer[],
   "up1"->Table[{ConvolutionLayer[c,{3,3},"PaddingSize"->1],BatchNormalizationLayer[],ParametricRampLayer[]},3],
   "output1"->{ConvolutionLayer[1,{1,1}],BatchNormalizationLayer[],Ramp},
   
   "resize2"->DeconvolutionLayer[c,{2,2},2],
   "cate2"->CatenateLayer[],
   "up2"->Table[{ConvolutionLayer[c,{3,3},"PaddingSize"->1],BatchNormalizationLayer[],ParametricRampLayer[]},3],
   "output2"->{ConvolutionLayer[1,{1,1}],BatchNormalizationLayer[],Ramp},
   
   "resize3"->DeconvolutionLayer[c,{2,2},2],
   "cate3"->CatenateLayer[],
   "up3"->Table[{ConvolutionLayer[c,{3,3},"PaddingSize"->1],BatchNormalizationLayer[],ParametricRampLayer[]},3],
   "output3"->{ResizeLayer[{301,621}],ConvolutionLayer[1,{1,1}],BatchNormalizationLayer[],Ramp}|>,
...
ReportSRnet[net_] :=
  Block[{obserP0,obserP1,obserP2,obserP3,simuP0,simuP1,simuP2,simuP3,simu,mse,corr},
        ...
        If[Total[mse]<Total[globalmse],
          Block[{},
                Export["/data/home/scy0446/run/Code/trained_"<>index<>".mx",net];
                (*
...
batch=128;
NetTrain[SRNet,
    {Function[Block[{Nmonth=8,m,d,tdynamics,p3,p2,p1,p0,tele},
    ...
    TrainingProgressReporting->{{Function@ReportSRnet[#Net], "Interval" -> Quantity[100, "Batches"]},"Print"}];
